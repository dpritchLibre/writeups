...
dplyr is one of the heaviest packages in the R ecosystem in terms of size and
build time

* As a consequence it should be have a higher utility threshold for inclusion as
  a package dependency
* Most dplyr functionality can be easily, albeit tediously, translated to the
  functions provided in the collection of packages that ship with R
...
to collect multiple values into a single entry for later use.
...
Another common task is to derive values based on multiple grouped variables.

* A helpful pattern is to create a data frame per key and write a function that
  operates on a single data frame
* The package tidyr offers the functions `nest` and `unnest` to aid the creation
  (and inverse operation) of creating such data frames
* tidyr is a component of the tidyverse and is the descendant of the reshape and
  reshape2 packages
...
## Creating nested data frames (cont.)

\small
```{r}
mtcars2_efficiency <- nest(grouped_mtcars2, mpg, hp, wt)
mtcars2_efficiency
```
...
\small
```{r}
model_mpg <- function(df) {
  out <- lm(mpg ~ hp + wt, df)
  coef(out)
} 
purrr::map(mtcars2_efficiency$data, model_mpg)
```
